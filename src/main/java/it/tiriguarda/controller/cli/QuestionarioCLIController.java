	...
	private Integer chiediDomanda(Scanner scanner, QuestionBean q) {
		while (true) {
			...
			for (int j = 0; j < opzioni.size(); j++) {
				ViewCLI.stampaMessaggio((j + 1) + " - " + opzioni.get(j));
			}
			
			ViewCLI.stampaMessaggio("Scegli un'opzione (1-" + opzioni.size() + "): ");
